...
use crate::{
    ...
    utils::*,
    ConfigLine, EndSettingType, LockupType, MagicHat, MagicHatData, MagicHatError,
...
pub struct MintNFT<'info> {
    ...
    // > Only needed if magic hat has lockup feature flag enabled
    // lockup_settings
    // token_manager
...
pub fn handle_mint_nft<'info>(
    ...
    if let Some(gatekeeper) = &magic_hat.data.gatekeeper {
        ...
        if gatekeeper.expire_on_use {
            ...
            {
                punish_bots(
                    MagicHatError::GatewayProgramError,
                    payer.to_account_info(),
                    ctx.accounts.magic_hat.to_account_info(),
                    ctx.accounts.system_program.to_account_info(),
                    BOT_FEE,
    ...
    let magic_hat_data = &magic_hat.data;
    let uuid = magic_hat_data.uuid.clone();
    if is_feature_active(&uuid, LOCKUP_SETTINGS_FEATURE_INDEX) {
        if ctx.remaining_accounts.len() <= remaining_accounts_counter {
            return err!(MagicHatError::LockupSettingsAccountMissing);
        }
        ...
        if lockup_settings.magic_hat != magic_hat.key() {
            return err!(MagicHatError::LockupSettingsAccountInvalid);
        }
...
fn handle_lockup_settings<'info>(
    ...
    if ctx.remaining_accounts.len() <= *remaining_accounts_counter {
        punish_bots(
            MagicHatError::LockupSettingsMissingAccounts,
            ctx.accounts.payer.to_account_info(),
            ctx.accounts.magic_hat.to_account_info(),
            ctx.accounts.system_program.to_account_info(),
            BOT_FEE,
    ...
    if ctx.remaining_accounts.len() <= *remaining_accounts_counter {
        return err!(MagicHatError::LockupSettingsMissingTokenManager);
    }
    let token_manager = &ctx.remaining_accounts[*remaining_accounts_counter];
    *remaining_accounts_counter += 1;

    if ctx.remaining_accounts.len() <= *remaining_accounts_counter {
        return err!(MagicHatError::LockupSettingsMissingTokenManagerTokenAccount);
    }
    let token_manager_token_account = &ctx.remaining_accounts[*remaining_accounts_counter];
    *remaining_accounts_counter += 1;

    if ctx.remaining_accounts.len() <= *remaining_accounts_counter {
        return err!(MagicHatError::LockupSettingsMissingMintCounter);
    }
    let mint_counter = &ctx.remaining_accounts[*remaining_accounts_counter];
    *remaining_accounts_counter += 1;

    if ctx.remaining_accounts.len() <= *remaining_accounts_counter {
        return err!(MagicHatError::LockupSettingsMissingRecipientTokenAccount);
    }
    let recipient_token_account = &ctx.remaining_accounts[*remaining_accounts_counter];
    *remaining_accounts_counter += 1;

    if ctx.remaining_accounts.len() <= *remaining_accounts_counter {
        return err!(MagicHatError::LockupSettingsMissingTokenManagerProgram);
    }
    ...
    if token_manager_program.key() != cardinal_token_manager::id() {
        return err!(MagicHatError::LockupSettingsInvalidTokenManagerProgram);
    }
    *remaining_accounts_counter += 1;
...
fn handle_time_invalidator<'info>(
    ...
    if ctx.remaining_accounts.len() <= *remaining_accounts_counter {
        return err!(MagicHatError::LockupSettingsMissingTimeInvalidator);
    }
    let time_invalidator = &ctx.remaining_accounts[*remaining_accounts_counter];
    *remaining_accounts_counter += 1;

    if ctx.remaining_accounts.len() <= *remaining_accounts_counter {
        return err!(MagicHatError::LockupSettingsMissingTimeInvalidatorProgram);
    }
    ...
    if time_invalidator_program.key() != cardinal_time_invalidator::id() {
        return err!(MagicHatError::LockupSettingsInvalidTimeInvalidatorProgram);
    }
    *remaining_accounts_counter += 1;
